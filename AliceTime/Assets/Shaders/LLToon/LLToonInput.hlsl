...
half4 _CameraReflectionTexture_TexelSize;
int _EnableMirror;
...
inline void InitializeStandardSurfaceDataLL(float2 uv, out SurfaceData outSurfaceData, float4 screenPosData)
{
    ...
    outSurfaceData.alpha = 0.5;//Alpha(albedoAlpha.a, _BaseColor, _Cutoff);
...
//Waterは自前のノーマル計算がある
    ...
    float2 uvApply = float2((uv.x - 0.5) * 30 + 0.5, (uv.y - 0.5) * 30 + 0.5);
    //finalColor.rgb += tex2D(_WaveTex, uv);
    //float2 scroll = float2(_SpeedX, _SpeedY) * _Time;
    //finalColor.rgb += SAMPLE_TEXTURE2D(_BumpMap, sampler_BumpMap, uv + scroll);
    
    float2 scroll = float2(_SpeedX, _SpeedY) * _Time;
    outSurfaceData.normalTS = SampleNormal(uvApply + scroll, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap), _BumpScale);
    outSurfaceData.normalTS += tex2D(_WaveTex, uvApply) * 1000;
//#endif
#else

...
public class ExplorePausePanel : ExplorePanelBehaviour
{
    [SerializeField]
    protected ExploreButtonBehaviour backButton = null;
    ...
    private void Update()
    {
        if (ExploreUIManager.CurrentPanel == this)
        {
            DefaultOperate();
            if (ExploreUIManager.uiPlayer.GetButton(CANCEL) || ExploreUIManager.uiPlayer.GetButton(PLUS))
            {
                if (backButton != null)
                {
